...
class SubsequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        step = int(self.data_array[y_idx][1])
        
        if step >= self.window_size:
            x = self.data_array[y_idx - self.window_size:y_idx, 3:]
        else:
            start = y_idx - step
            core = self.data_array[start:y_idx, 3:]
            pad = np.zeros((self.window_size - step, core.shape[1]), dtype=self.data_array.dtype)
            x = np.vstack((pad, core))
