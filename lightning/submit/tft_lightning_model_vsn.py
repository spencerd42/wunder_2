...
class VariableSelectionNetwork(nn.Module):
    """Optimized VSN with shared representations"""
    def __init__(self, input_size, hidden_size, dropout=0.1):
        super().__init__()
        # Single GRN for all features (not one per feature)
        ...
        # Shared projection instead of individual GRNs
        self.feature_projection = nn.Linear(input_size, hidden_size)
        
    def forward(self, x):
        # x shape: (batch, seq_len, input_size)
        # Compute feature weights
        weights = self.softmax(self.flattened_grn(x))  # (batch, seq_len, input_size)
        
        # Weight the input features
        weighted = x * weights  # Element-wise multiplication
        
        # Project to hidden dimension
        return self.feature_projection(weighted)  # (batch, seq_len, hidden_size)
...
class TemporalFusionTransformerLightning(pl.LightningModule):
    ...
    def __init__(
        ...
        # Variable Selection Network instead of simple projection
        self.variable_selection_network = VariableSelectionNetwork(
            input_size=input_size,
            hidden_size=hidden_size,
            dropout=dropout
        )
        ...
        # Self-attention with updated dimension (hidden_size * 2 for bidirectional)
        ...
        # Layer norm and dropout adapted for bidirectional output size
    ...
    def forward(self, x):
        ...
        x_proj = self.variable_selection_network(x)
